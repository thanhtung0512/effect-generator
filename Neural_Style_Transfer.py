...
def transfer_style(cnn_path, 
    ...
    print(f"Content image path: {cimg}")
...
def main():
    ...
    content_images = sorted(Path(args.content_imgs).glob("*.jpg"))
    print(f'path content images {Path(args.content_imgs)}')
    # with open(args.content_imgs, "r", encoding="latin-1") as f:
    #     lines = f.read()
    # content_images = lines.split('\n')
    # content_images = [Path("/home/thanhtung/Downloads/archive/Weather_Effect_Generator/test-img") / f for f in content_images]
    ...
    for cimg in tqdm(content_images):
        print(f'cimg: {cimg}')
        name, extension = os.path.splitext(cimg.name)
