...
def getDisparityMap(model, transform, img_path):
    if os.path.isdir(img_path):
        # If img_path is a directory, load all image files within it
        image_files = sorted(Path(img_path).glob("*"))
        disparity_maps = []
        for image_file in image_files:
            ...
            with torch.no_grad():
                ...
                prediction = torch.nn.functional.interpolate(
                    ...
                    size=img_np.shape[:2],  # Use img_np.shape instead of img.shape
                    mode="bicubic",
                    align_corners=False,
                ).squeeze()
            disparity_maps.append(prediction.cpu().numpy())
        return disparity_maps
    else:
        # If img_path is a single image file, process it as before
...
def main():
    ...
    if imgP.is_file():
        ...
        # Construct the depth map file name by concatenating the stem of the image file with the ".npy" extension
        depth_file_name = imgP.stem + ".jpg.npy"
        np.save(save_folder / depth_file_name, depth)
